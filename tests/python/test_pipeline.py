...
_STATION = 60003760
_DOCKED_STATION = 60000004  # where the character is actually docked
_OWNER = 1000035
_FACTION = 500001
...
def _handler(request: httpx.Request) -> httpx.Response:
    ...
    if path.endswith("/markets/prices/"):
        return httpx.Response(
            200,
            json=[
                {"type_id": _UNDERVALUED, "average_price": 700.0, "adjusted_price": 720.0},
                {"type_id": _HELD_DEAR, "average_price": 1400.0, "adjusted_price": 1400.0},
            ],
            headers=exp,
        )
    ...
    if path.endswith("/location/"):
        return httpx.Response(
            200, json={"solar_system_id": 30000142, "station_id": _DOCKED_STATION}, headers=exp
        )
    if path.endswith("/ship/"):
        return httpx.Response(
            200,
            json={"ship_type_id": 587, "ship_item_id": 900, "ship_name": "My Ship"},
            headers=exp,
        )
    if path.endswith("/characters/affiliation/"):
        return httpx.Response(
            200, json=[{"character_id": 42, "corporation_id": 98000001}], headers=exp
        )
    ...
    if "/universe/names/" in path:
        catalogue = {
            ...
            _HELD_DEAR: "Pyerite",
            _STATION: "Jita IV-4",
            _DOCKED_STATION: "Amarr VIII - Emperor Family Academy",
...
def test_reference_prices_prefer_average_and_fall_back_to_adjusted() -> None:
    prices = [
        MarketPrice(type_id=34, average_price=5.0, adjusted_price=6.0),
        MarketPrice(type_id=35, adjusted_price=9.0),  # no average -> use adjusted
        MarketPrice(type_id=36),  # neither -> omitted
    ]
    assert _reference_prices(prices) == {34: 5.0, 35: 9.0}
    ...
    async def go() -> None:
        ...
        async with httpx.AsyncClient(transport=transport) as http:
            ...
            character = await fetch_character(
                client, authenticator, _config(), 42, name_cache, structure_cache, now=now
            )
            ...
            # The header shows where the character actually is docked.
            assert character.current_location_name == "Amarr VIII - Emperor Family Academy"
